#!/usr/bin/env/python3
"""Recipe for predict a phoneme recognizer with
Transducer  on the TIMIT dataset.

To run this recipe, do the following:
> python predict.py hparams/train.yaml --data_folder /path/to/TIMIT
...
class ASR_Brain(sb.Brain):
    def infer(self, wavs, wav_lens, stage):
...
class Predictor(object):
    def __init__(self, params):
        hparams_file, run_opts, overrides = sb.parse_arguments(params)
        ...
        self.asr_brain.label_encoder = label_encoder
        self.asr_brain.on_evaluate_start(max_key=None, min_key='PER')
        self.asr_brain.on_stage_start(sb.Stage.TEST, epoch=None)
        self.asr_brain.modules.eval()

    def predict(self, f_path):
        test_sig = sb.dataio.dataio.read_audio(f_path)
        test_sig_len = test_sig.shape[0]
        test_sig = torch.Tensor([test_sig.numpy()])
        test_sig_len = torch.FloatTensor([1.])
        test_res = self.asr_brain.infer(test_sig, test_sig_len, 'test')
        res = []
        for elem in test_res[0]:
            elem_res = [self.asr_brain.label_encoder.ind2lab[x] for x in elem]
            res.append(elem_res)
        return res 
...
if __name__ == "__main__":
    ...
    # Test
    test_path = '/workspace/sb/recipes/TIMIT/data/TIMIT/test/dr8/fcmh1/sx143.wav'
    test_res = predictor.predict(test_path)
    print(test_res)
